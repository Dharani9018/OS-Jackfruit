...
static void timer_callback(struct timer_list *t) {
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (rss < 0) {
            ...
            printk(KERN_INFO "[container_monitor] Removing stale entry pid=%d\n", entry->pid);
            list_del(&entry->list);
            kfree(entry);
            continue;
        }
        ...
        /* Check soft limit */
        if ((unsigned long)rss > entry->soft_limit_bytes && !entry->soft_limit_warned) {
